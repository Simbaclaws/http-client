...
class HTTPClient {
    ...
    constructor(baseUrl, authManager) {
        this.hasWarnedAboutNoAuth = false;
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (true) {
                case contentType.includes('application/json'):
                    return yield response.json(); // Returns a parsed JSON object or array.
                ...
                case contentType.includes('text/yaml'):
                    return yield response.text(); // Returns a string.
                case contentType.includes('application/xml'):
                case contentType.includes('text/xml'):
                    const xmlText = yield response.text();
                    return new window.DOMParser().parseFromString(xmlText, 'application/xml'); // Returns an XMLDocument.
                case contentType.includes('multipart/form-data'):
                    return yield response.formData(); // Returns FormData.
                case contentType.includes('application/x-www-form-urlencoded'):
                    const formText = yield response.text();
                    return new URLSearchParams(formText); // Returns URLSearchParams.
                case contentType.includes('application/octet-stream'):
                    return yield response.arrayBuffer(); // Returns an ArrayBuffer.
                ...
                case contentType.includes('application/rtf'):
                    return yield response.blob(); // Returns a Blob.
                default:
                    console.warn(`Unhandled content type: ${contentType}`);
                    return yield response.text(); // Default to string if content type is unrecognized.
            }
        });
    }
    /**
    ...
    */
    ...
    request(method_1, endpoint_1, body_1) {
        return __awaiter(this, arguments, void 0, function* (method, endpoint, body, headers = {}) {
            ...
            try {
                ...
                yield this.handleStatusCode(response);
                return yield this.handleContentType(response); // Directly call handleContentType
            }
            catch (error) {
     ...
     */
    headRequest(method_1, endpoint_1) {
        return __awaiter(this, arguments, void 0, function* (method, endpoint, headers = {}) {
            const url = `${this.baseUrl}${endpoint}`;
            const options = {
                method,
